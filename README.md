...
### Database
Database is from Official NASA Exoplanets Archive web downloaded as csv and preprocessed in Python Pandas to format compatible with PostgreSQL. And another csv table from adityamishraml on kaggle.com, which is also from NASA Exoplanets Archive, but with more decribing columns which are usefull. I connect these two tables in PostgreSQL into one table exoplanets and created dimensional tables to exoplanets table.
